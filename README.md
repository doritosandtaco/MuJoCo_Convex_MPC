...
(이미지가 보이지 않는다면 동일한 경로에 trot_forward.gif가 있는지 확인하세요)
...
(이미지가 보이지 않는다면 동일한 경로에 trot_forward_push.gif가 있는지 확인하세요)
...
4. 시뮬레이션 결과 GIF 녹화
카메라가 로봇을 추적하며 렌더링된 결과를 GIF로 자동 저장합니다.

Bash
python run_go2_convex_mpc.py --mode trot --vx 0.4 --record trot_forward.gif
📁 Project Structure
run_go2_convex_mpc.py: 시뮬레이션 실행 및 뷰어, 카메라 렌더링을 담당하는 메인 엔트리 스크립트입니다.

controller.py: Gait 스케줄러를 기반으로 Stance 발의 MPC 토크와 Swing 발의 PD 토크를 계산하고 적용하는 핵심 제어기입니다.

mpc.py: 로봇의 동역학 모델을 상태 공간(State-Space) 행렬로 구성하고 OSQP를 통해 최적의 지면 반력을 도출합니다.

gait.py: Trot 보행의 위상(Phase)과 듀티 사이클(Duty Factor)을 관리합니다.

config.py: MPC의 Cost Weight Matrix (Q행렬) 및 제어 주파수, Raibert Heuristic 게인 등 튜닝 파라미터를 통합 관리합니다.

📝 Future Work (To-Do)
[ ] Yaw 회전 주행 (Steering) 최적화

[ ] 경사면(Slope) 및 계단 보행 제어 추가
